 ...
 */
@Controller
public class SystemdJournalTest implements CommandLineRunner {
    ...
    private void processLoop(int loop) {
        ...
        //logger.info("Existing cursor: " + existingCursor.getCString());
        //logger.info("New cursor: " + newCursor.getCString());

        if (existingCursor.getCString().equals(newCursor.getCString())) {
            //logger.info("Cursors match");
        } else {
            logger.info("Cursors don't match");
